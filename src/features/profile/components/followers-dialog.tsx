...
export default function FollowersDialog({ trigger }: FollowersDialogProps) {
    ...
    const { data, isLoading, hasNextPage, fetchNextPage } = useInfiniteGetFollowersQuery(
        ...
        },
        {
            ...
            getNextPageParam: (lastPage, pages) => {
                ...
                return lastPage.user?.followers.length === MAX_LIMIT ? { followers_offset: nextOffset } : undefined;
            },
            enabled: !!userId
        }
    );
    ...
    const items = useMemo(() => {
        if (!data) return [];
        return data.pages.flatMap((page) => page.user?.followers || []);
    }, [data]);

    return (
        <ResponsiveDialog title='Followers' trigger={trigger} className='max-h-[80vh]' hasVisibleTitle>
            <div className='flex flex-col gap-4'>
                {isLoading ? (
                    ...
                    <p className='text-muted-foreground text-center'>No followers found</p>
                ) : (
                    <div className='flex flex-col gap-4'>
                        {items.map((item) => {

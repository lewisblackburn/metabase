// https://lwmecktyyhputyqkdigy.functions.eu-west-2.nhost.run/v1/log_user_book_status_activity
import {
    Activity_Types_Enum,
    GetBookTitleDocument,
    GetBookTitleQuery,
    GetBookTitleQueryVariables,
...
export default async function (req: Request, res: Response) {
    try {
        ...
        const { books } = await client.request<GetBookTitleQuery, GetBookTitleQueryVariables>(GetBookTitleDocument, {
            book_id: newData.book_id
        });
        const bookTitle = books[0]?.title;
        if (!bookTitle) return res.status(404).json({ error: 'Book not found' });
        ...
        for (const activityType of activitiesToLog) {
            await client.request<InsertUserActivityMutation, InsertUserActivityMutationVariables>(
                ...
                {
                    object: {
                        object_id: newData.book_id,
                        object_title: bookTitle,
        ...
        console.error('log_user_book_status_activity error:', err);

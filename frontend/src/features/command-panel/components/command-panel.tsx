...
export default function CommandPanel() {
    ...
    const handleSelect = useCallback(
        (item: ItemType | { id?: string; type?: string | null; href?: string }) => {
            if (!item.id) return;
    ...
    const filteredActions = isSearching
        ? ALL_ACTIONS.filter((action) => action.title.toLowerCase().includes(query.toLowerCase()))
        : ALL_ACTIONS;
        ...
        <CommandDialog open={isOpen} onOpenChange={() => dispatch(toggleCommandPanelOpenState())}>
            <Command>
                ...
                <CommandList>
                    ...
                    <CommandGroup heading={isSearching ? 'Matching actions' : 'All actions'} value='actions'>
                        {filteredActions.map((item, index) => (
                            <Item key={index} item={item} onSelect={() => handleSelect(item)} />
                        ))}
                    </CommandGroup>
                </CommandList>
            </Command>

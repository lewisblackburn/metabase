...
import { Order_By, useInfiniteGetSongReviewsQuery } from '@/generated/graphql';
...
export default function SongReviews() {
    ...
    const { data, isLoading, hasNextPage, fetchNextPage } = useInfiniteGetSongReviewsQuery(
        {
            where: {
                song_id: {
                    _eq: params?.id
        ...
        },
        {
            ...
            getNextPageParam: (lastPage, pages) => {
                ...
                return lastPage.user_song_statuses.length === MAX_LIMIT ? { offset: nextOffset } : undefined;
            }
        }
    );
    ...
    const allReviews = useMemo(() => {
        return data?.pages.flatMap((page) => page.user_song_statuses) || [];
    }, [data]);

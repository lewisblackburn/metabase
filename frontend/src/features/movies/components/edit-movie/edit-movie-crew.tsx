...
export default function EditMovieCrew({ movieId }: { movieId: string }) {
    ...
    const { data, isLoading } = useGetCreditsQuery(
        ...
        {
            queryKey: ['movie-crew', movieId, debouncedSearchQuery, pageIndex, pageSize, sorting]
        }
    );

    const crewMembers = data?.credits ?? [];
    const totalRows = data?.credits_aggregate?.aggregate?.count ?? 0;

    const { mutateAsync: deleteMovieCredits } = useDeleteCreditsMutation({
        onSuccess: () => {
            toast.success('Crew members deleted successfully');
    ...
    const handleRowOrderChange = async (newData: typeof crewMembers) => {
        const movedItem = newData.find((item, index) => item.id !== crewMembers[index]?.id);
        if (!movedItem) return;
    ...
    return (
        <div className='overflow-x-auto'>
            <div className='mb-4 flex w-full items-center justify-between gap-2'>
                <Input
                    placeholder='Search by crew member name or job...'
                    value={searchQuery}
                    onChange={(e) => {
                ...
                <AddCrewMemberDialog movieId={movieId} />
